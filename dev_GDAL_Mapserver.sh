...
if [ -z "${goto_mapserver_build:-}" ]; then
	...
	ctest -E "(bench|benchmark|autotest.*bench|autotest_gcore)" --output-on-failure -j"$THREADS" || {
...
# MapServer
#############################################
progress "Building MAPSERVER dev ..."
cd "$PACKAGE_DIR"
